...
@interface CPTalkView () <CPErrorViewDelegate,
						  HttpRequestDelegate,
...
- (void)requestItems:(NSNumber *)ignoreCache
{
	...
	NSString *url = [[Modules urlWithQueryString:_talkDic[@"url"]] stringByAppendingFormat:@"&requestTime=%@",
...
- (NSString *)getViewWiselogCode:(NSString *)type
{
	...
	if ([type isEqualToString:@"commonMoreView"])	code = @"MAF0400";
	else if ([type isEqualToString:@"lineBanner"])	code = @"MAF0500";
