...
@interface CPMartView () <	CPErrorViewDelegate,
							HttpRequestDelegate,
							...
							UICollectionViewDelegate  >
{
	...
	NSDictionary *_item;
	NSMutableDictionary *_topMartAreaItem;
...
- (id)initWithFrame:(CGRect)frame
{
	if (self = [super initWithFrame:frame])
	{
		...
		_topMartAreaItem = [NSMutableDictionary dictionary];
...
- (void)requestItems:(NSNumber *)ignoreCache
{
	...
	requestSuccess = ^(NSDictionary *requestData) {
		...
		if (_topMartAreaItem) {
			[_topMartAreaItem removeAllObjects];
		}
		...
		if (dataArray) {
			...
			for (NSInteger i=0; i<[dataArray count]; i++) {
				...
				if ([@"topMartArea" isEqualToString:groupName]) {
					_topMartAreaItem = [dataArray[i] mutableCopy];
					break;
				}
			}
		}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([kind isEqual:UICollectionElementKindSectionHeader]) {
		...
		//메뉴
		NSArray *tapItems = _topMartAreaItem[@"topMartArea"];
		for (NSDictionary *dic in tapItems) {
			...
			if ([tapItems indexOfObject:dic] == 0)		imageName = @"img_mart_big1.png";
			else if ([tapItems indexOfObject:dic] == 1) imageName = @"img_mart_big2.png";
			else if ([tapItems indexOfObject:dic] == 3) imageName = @"img_mart_big3.png";
			else										imageName = @"img_mart_big4.png";
