...
@interface CPProductSellPeriodView()
{
    NSDictionary *product;
    NSDictionary *sellPeriodInfo;
}

@end

@implementation CPProductSellPeriodView

- (void)releaseItem
{
    if (product)        product = nil;
    if (sellPeriodInfo) sellPeriodInfo = nil;
}
...
- (id)initWithFrame:(CGRect)frame product:(NSDictionary *)aProduct
{
    if (self = [super initWithFrame:frame]) {
        ...
        if (product[@"prdSalPeriod"]) {
            sellPeriodInfo = [product[@"prdSalPeriod"] copy];
...
- (void)initLayout
{
    ...
    NSString *title = sellPeriodInfo[@"label"];
    ...
    NSString *text = sellPeriodInfo[@"text"];
