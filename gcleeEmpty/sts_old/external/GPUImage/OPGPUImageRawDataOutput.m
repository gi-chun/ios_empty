...
@interface OPGPUImageRawDataOutput ()
{
...
@implementation OPGPUImageRawDataOutput
...
- (id)initWithImageSize:(CGSize)newImageSize resultsInBGRAFormat:(BOOL)resultsInBGRAFormat;
{
    ...
    if ( (outputBGRA && ![OPGPUImageContext supportsFastTextureUpload]) || (!outputBGRA && [OPGPUImageContext supportsFastTextureUpload]) )
    {
        dataProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImageColorSwizzlingFragmentShaderString];
    }
    else
    {
        dataProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImagePassthroughFragmentShaderString];
    }
    ...
    [OPGPUImageContext setActiveShaderProgram:dataProgram];
...
- (void)dealloc
{
    ...
    if (_rawBytesForImage != NULL && (![OPGPUImageContext supportsFastTextureUpload])) 
    {
...
- (void)createDataFBO;
{
    ...
    if ([OPGPUImageContext supportsFastTextureUpload])
    {
        ...
        CVReturn err = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, [[OPGPUImageContext sharedImageProcessingContext] context], NULL, &rawDataTextureCache);
#else
        CVReturn err = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, (__bridge void *)[[OPGPUImageContext sharedImageProcessingContext] context], NULL, &rawDataTextureCache);
#endif
#else
        CGLContextObj context = (__bridge void *)[[OPGPUImageContext sharedImageProcessingContext] context];
...
{
    OPGPUByteColorVector *imageColorBytes = (OPGPUByteColorVector *)self.rawBytesForImage;
//    NSLog(@"Row start");
...
//    NSLog(@"Byte 1: %d, %d, %d, byte 2: %d, %d, %d, byte 3: %d, %d, %d", byteAtOne.red, byteAtOne.green, byteAtOne.blue, byteAtWidth.red, byteAtWidth.green, byteAtWidth.blue, byteAtHeight.red, byteAtHeight.green, byteAtHeight.blue);
    ...
    if (outputBGRA)    
    {
        OPGPUByteColorVector flippedColor = imageColorBytes[(int)(round((locationToPickFrom.y * imageSize.width) + locationToPickFrom.x))];
