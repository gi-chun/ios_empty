...
@implementation OPGPUImageFilterGroup
...
- (void)setTargetToIgnoreForUpdates:(id<OPGPUImageInput>)targetToIgnoreForUpdates;
{
...
- (void)setFrameProcessingCompletionBlock:(void (^)(OPGPUImageOutput *, CMTime))frameProcessingCompletionBlock;
{
    [_terminalFilter setFrameProcessingCompletionBlock:frameProcessingCompletionBlock];
}

- (void (^)(OPGPUImageOutput *, CMTime))frameProcessingCompletionBlock;
{
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    for (OPGPUImageOutput<OPGPUImageInput> *currentFilter in _initialFilters)
    {
...
{
    for (OPGPUImageOutput<OPGPUImageInput> *currentFilter in filters)
    {
...
- (void)endProcessing
{
//	NSLog(@"isEndProcessing : %@", self.isEndProcessing ? @"YES" : @"NO");
	...
	if (!self.isEndProcessing)
    {
		...
		for (OPGPUImageOutput<OPGPUImageInput> *currentFilter in _initialFilters)
		{
//			NSLog(@"currentFilter desc : %@", [[currentFilter class] description]);
			[currentFilter endProcessing];
		}
    }
}
