...
@interface OPGPUImageView () 
{
    GLuint inputTextureForDisplay;
    GLuint displayRenderbuffer, displayFramebuffer;
    
    OPGLProgram *displayProgram;
...
- (void)commonInit;
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        displayProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImagePassthroughFragmentShaderString];
        ...
        [OPGPUImageContext setActiveShaderProgram:displayProgram];
        glEnableVertexAttribArray(displayPositionAttribute);
        glEnableVertexAttribArray(displayTextureCoordinateAttribute);
        
        [self setBackgroundColorRed:0.0 green:0.0 blue:0.0 alpha:1.0];
        _fillMode = kOPGPUImageFillModePreserveAspectRatio;
...
- (void)createDisplayFramebuffer;
{
	...
	[[[OPGPUImageContext sharedImageProcessingContext] context] renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer*)self.layer];
...
{
    glBindRenderbuffer(GL_RENDERBUFFER, displayRenderbuffer);
    [[OPGPUImageContext sharedImageProcessingContext] presentBufferForDisplay];
}
...
- (void)recalculateViewGeometry;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        switch(_fillMode)
        {
            case kOPGPUImageFillModeStretch:
            {
                widthScaling = 1.0;
                heightScaling = 1.0;
            }; break;
            case kOPGPUImageFillModePreserveAspectRatio:
            {
                widthScaling = insetRect.size.width / currentViewSize.width;
                heightScaling = insetRect.size.height / currentViewSize.height;
            }; break;
            case kOPGPUImageFillModePreserveAspectRatioAndFill:
            {
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        glVertexAttribPointer(displayTextureCoordinateAttribute, 2, GL_FLOAT, 0, 0, [OPGPUImageView textureCoordinatesForRotation:inputRotation]);
...
- (void)setFillMode:(OPGPUImageFillModeType)newValue;
{
    _fillMode = newValue;
    [self recalculateViewGeometry];
}
