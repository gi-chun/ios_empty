...
@implementation OPGPUImageContext
...
- (id)init;
{
    ...
    _contextQueue = dispatch_queue_create("com.futurewiz.11stVideo.openGLESContextQueue", NULL);
...
+ (OPGPUImageContext *)sharedImageProcessingContext;
{
    ...
    static OPGPUImageContext *sharedImageProcessingContext = nil;
...
{
    EAGLContext *imageProcessingContext = [[OPGPUImageContext sharedImageProcessingContext] context];
...
{
    OPGPUImageContext *sharedContext = [OPGPUImageContext sharedImageProcessingContext];
...
+ (BOOL)deviceSupportsRedTextures;
{
    ...
    dispatch_once(&pred, ^{
        supportsRedTextures = [OPGPUImageContext deviceSupportsOpenGLESExtension:@"GL_EXT_texture_rg"];
    });
...
+ (BOOL)deviceSupportsFramebufferReads;
{
    ...
    dispatch_once(&pred, ^{
        supportsFramebufferReads = [OPGPUImageContext deviceSupportsOpenGLESExtension:@"GL_EXT_shader_framebuffer_fetch"];
    });
...
- (OPGLProgram *)programForVertexShaderString:(NSString *)vertexShaderString fragmentShaderString:(NSString *)fragmentShaderString;
{
    ...
    OPGLProgram *programFromCache = [shaderProgramCache objectForKey:lookupKeyForShaderProgram];

    if (programFromCache == nil)
    {
        programFromCache = [[OPGLProgram alloc] initWithVertexShaderString:vertexShaderString fragmentShaderString:fragmentShaderString];
...
+ (void)resetSharedContextQueue {
	[[self sharedImageProcessingContext] resetContextQueue];
}
