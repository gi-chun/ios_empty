...
const GLfloat kOPColorConversion601[] = {
    1.164,  1.164, 1.164,
    0.0, -0.392, 2.017,
    1.596, -0.813,   0.0,
};
...
const GLfloat kOPColorConversion709[] = {
    1.164,  1.164, 1.164,
    0.0, -0.213, 2.112,
    1.793, -0.533,   0.0,
};

NSString *const kOPGPUImageYUVVideoRangeConversionForRGFragmentShaderString = OPSHADER_STRING
(
...
NSString *const kOPGPUImageYUVVideoRangeConversionForLAFragmentShaderString = OPSHADER_STRING
(
 varying highp vec2 textureCoordinate;
...
@interface OPGPUImageVideoCamera () 
{
...
@implementation OPGPUImageVideoCamera
...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition; 
{
    ...
    _runBenchmark = NO;
    capturePaused = NO;
    outputRotation = kOPGPUImageNoRotation;
    captureAsYUV = YES;
...
//            {
//                yuvConversionProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImageYUVVideoRangeConversionForRGFragmentShaderString];
//            }
//            else
//            {
...
//    if (captureAsYUV && [OPGPUImageContext deviceSupportsRedTextures])
    if (captureAsYUV && [OPGPUImageContext supportsFastTextureUpload])
    {
...
- (void)addTarget:(id<OPGPUImageInput>)newTarget atTextureLocation:(NSInteger)textureLocation;
{
...
- (BOOL)isBackFacingCameraPresent
{
    return [OPGPUImageVideoCamera isBackFacingCameraPresent];
}
...
- (BOOL)isFrontFacingCameraPresent
{
    return [OPGPUImageVideoCamera isFrontFacingCameraPresent];
}
...
#define OPINITIALFRAMESTOIGNOREFORBENCHMARK 5
...
//                err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, coreVideoTextureCache, cameraFrame, NULL, GL_TEXTURE_2D, GL_RG_EXT, bufferWidth/2, bufferHeight/2, GL_RG_EXT, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
        ...
        if (_runBenchmark)
        {
            ...
            if (numberOfFramesCaptured > OPINITIALFRAMESTOIGNOREFORBENCHMARK)
            {
...
{
    return (totalFrameTimeDuringCapture / (CGFloat)(numberOfFramesCaptured - OPINITIALFRAMESTOIGNOREFORBENCHMARK)) * 1000.0;
}
...
- (void)setAudioEncodingTarget:(OPGPUImageMovieWriter *)newValue;
{
    if (newValue) {
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([self cameraPosition] == AVCaptureDevicePositionBack)
        {
            if (_horizontallyMirrorRearFacingCamera)
            {
                ...
                {
                    case UIInterfaceOrientationPortrait:outputRotation = kOPGPUImageRotateRightFlipVertical; break;
                    case UIInterfaceOrientationPortraitUpsideDown:outputRotation = kOPGPUImageRotate180; break;
                    case UIInterfaceOrientationLandscapeLeft:outputRotation = kOPGPUImageFlipHorizonal; break;
                    case UIInterfaceOrientationLandscapeRight:outputRotation = kOPGPUImageFlipVertical; break;
                }
            }
            else
            {
                switch(_outputImageOrientation)
                {
                    case UIInterfaceOrientationPortrait:outputRotation = kOPGPUImageRotateRight; break;
                    case UIInterfaceOrientationPortraitUpsideDown:outputRotation = kOPGPUImageRotateLeft; break;
                    case UIInterfaceOrientationLandscapeLeft:outputRotation = kOPGPUImageRotate180; break;
                    case UIInterfaceOrientationLandscapeRight:outputRotation = kOPGPUImageNoRotation; break;
                }
            }
        }
        else
        {
            if (_horizontallyMirrorFrontFacingCamera)
            {
                ...
                {
                    case UIInterfaceOrientationPortrait:outputRotation = kOPGPUImageRotateRightFlipVertical; break;
                    case UIInterfaceOrientationPortraitUpsideDown:outputRotation = kOPGPUImageRotateRightFlipHorizontal; break;
                    case UIInterfaceOrientationLandscapeLeft:outputRotation = kOPGPUImageFlipHorizonal; break;
                    case UIInterfaceOrientationLandscapeRight:outputRotation = kOPGPUImageFlipVertical; break;
                }
            }
            else
            {
                switch(_outputImageOrientation)
                {
                    case UIInterfaceOrientationPortrait:outputRotation = kOPGPUImageRotateRight; break;
                    case UIInterfaceOrientationPortraitUpsideDown:outputRotation = kOPGPUImageRotateLeft; break;
                    case UIInterfaceOrientationLandscapeLeft:outputRotation = kOPGPUImageNoRotation; break;
                    case UIInterfaceOrientationLandscapeRight:outputRotation = kOPGPUImageRotate180; break;
                }
            }
        }
