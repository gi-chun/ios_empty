...
@interface OPGPUImageRawDataInput()
- (void)uploadBytes:(GLubyte *)bytesToUpload;
@end

@implementation OPGPUImageRawDataInput
...
{
    if (!(self = [self initWithBytes:bytesToUpload size:imageSize pixelFormat:OPGPUPixelFormatBGRA type:OPGPUPixelTypeUByte]))
    {
		return nil;
    }
...
{
    if (!(self = [self initWithBytes:bytesToUpload size:imageSize pixelFormat:pixelFormat type:OPGPUPixelTypeUByte]))
    {
		return nil;
    }
...
- (void)uploadBytes:(GLubyte *)bytesToUpload;
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, _pixelFormat==OPGPUPixelFormatRGB ? GL_RGB : GL_RGBA, (int)uploadedImageSize.width, (int)uploadedImageSize.height, 0, (GLint)_pixelFormat, (GLenum)_pixelType, bytesToUpload);
}
