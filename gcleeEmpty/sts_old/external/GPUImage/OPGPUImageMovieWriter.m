...
NSString *const kOPGPUImageColorSwizzlingFragmentShaderString = OPSHADER_STRING
(
...
@interface OPGPUImageMovieWriter ()
{
    GLuint movieFramebuffer, movieRenderbuffer;
    
    OPGLProgram *colorSwizzlingProgram;
...
@implementation OPGPUImageMovieWriter
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    movieWritingQueue = dispatch_queue_create("com.futurewiz.11stVideo.movieWritingQueue", NULL);
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        {
            colorSwizzlingProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImagePassthroughFragmentShaderString];
        }
        else
        {
            colorSwizzlingProgram = [[OPGPUImageContext sharedImageProcessingContext] programForVertexShaderString:kOPGPUImageVertexShaderString fragmentShaderString:kOPGPUImageColorSwizzlingFragmentShaderString];
        }
        ...
        [OPGPUImageContext setActiveShaderProgram:colorSwizzlingProgram];
...
- (void)initializeMovieWithOutputSettings:(NSDictionary *)outputSettings;
{
	...
	NSMutableDictionary *tmpOutputSettings = [NSMutableDictionary dictionaryWithDictionary:outputSettings];
	...
	// MPEG4 인코딩 관련 프로파일 변경
	int bitrate = 2000000;
	if ([CPVideoCompositionItem currentCompositionItem].frameSize == CPMovieFrameSize480x480) {
		// 480x480일 때는 bitrate를 1.5mbps로 설정
		bitrate = 1500000;
	}
	
	// * 참고 :
	//   bitrate는 h.264의 VBR 세부 설정이 불가
	//   min, max는 iOS 상에서 자체적으로 해주는 듯
	//   ABR 값을 통하여 평균 값 컨트롤만 가능
	
    NSMutableDictionary * compressionProperties = [[NSMutableDictionary alloc] init];
    [compressionProperties setObject:videoCleanApertureSettings forKey:AVVideoCleanApertureKey];
    [compressionProperties setObject:videoAspectRatioSettings forKey:AVVideoPixelAspectRatioKey];
    [compressionProperties setObject:[NSNumber numberWithInt: bitrate] forKey:AVVideoAverageBitRateKey];
    [compressionProperties setObject:[NSNumber numberWithInt: 16] forKey:AVVideoMaxKeyFrameIntervalKey];
    [compressionProperties setObject:AVVideoProfileLevelH264Baseline30 forKey:AVVideoProfileLevelKey];
    
    [tmpOutputSettings setObject:compressionProperties forKey:AVVideoCompressionPropertiesKey];
	
	outputSettings = [NSDictionary dictionaryWithDictionary:tmpOutputSettings];
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        if( assetWriter.status != AVAssetWriterStatusWriting )
        {
			if (assetWriter.status == AVAssetWriterStatusFailed) {
				...
				UIAlertView *alertView = [UIAlertView alertViewWithTitle:nil
																 message:@"이 얼럿창을 보는 분은\n필히 주찬이에게 알려주세요..."
													   cancelButtonTitle:nil
													   otherButtonTitles:[NSArray arrayWithObjects:@"닫기", nil]
															   onDismiss:nil
																onCancel:nil];
				[alertView show];
				 */
        ...
        videoQueue = dispatch_queue_create("com.futurewiz.11stVideo.videoReadingQueue", NULL);
    ...
    if (audioInputReadyCallback != NULL)
    {
        audioQueue = dispatch_queue_create("com.futurewiz.11stVideo.audioReadingQueue", NULL);
...
- (void)renderAtInternalSize;
{
    ...
    const GLfloat *textureCoordinates = [OPGPUImageFilter textureCoordinatesForRotation:inputRotation];
...
- (void)setHasAudioTrack:(BOOL)newValue audioSettings:(NSDictionary *)audioOutputSettings;
{
    ...
    if (_hasAudioTrack)
    {
        ...
        {
			double preferredHardwareSampleRate = 0.f;
