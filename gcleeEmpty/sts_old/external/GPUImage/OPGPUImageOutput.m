...
#else
	if (dispatch_get_specific([OPGPUImageContext contextKey]))
#endif
	{
		block();
	}else
...
@implementation OPGPUImageOutput
...
- (void)removeTarget:(id<OPGPUImageInput>)targetToRemove;
{
    ...
    runSynchronouslyOnVideoProcessingQueue(^{
		...
		[targetToRemove setInputRotation:kOPGPUImageNoRotation atIndex:textureIndexOfTarget];
...
- (void)removeAllTargets;
{
    cachedMaximumOutputSize = CGSizeZero;
    runSynchronouslyOnVideoProcessingQueue(^{
        for (id<OPGPUImageInput> targetToRemove in targets)
        {
            NSInteger indexOfObject = [targets indexOfObject:targetToRemove];
            NSInteger textureIndexOfTarget = [[targetTextureIndices objectAtIndex:indexOfObject] integerValue];
...
- (CGImageRef)newCGImageByFilteringCGImage:(CGImageRef)imageToFilter orientation:(UIImageOrientation)orientation;
{
    ...
    [stillImageSource addTarget:(id<OPGPUImageInput>)self];
    [stillImageSource processImage];
    
    CGImageRef processedImage = [self newCGImageFromCurrentlyProcessedOutputWithOrientation:orientation];
    
    [stillImageSource removeTarget:(id<OPGPUImageInput>)self];
